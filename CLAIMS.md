...
| A8 | **Three families, one configuration, no per-model tuning** through the shipped API: Llama-3.2-1B (GQA 32:8) 1.007 aggregate / 1.000 worst task; Qwen2.5-3B (GQA 16:2) 0.975 / 0.917; Phi-3.5-mini (MHA 32:32 + LongRoPE) 0.972 / 0.889. The weak task on every model is niah_multikey_3, where the models themselves are weakest | `artifacts/bounded-decode-frontier-multimodel-{llama1b,qwen2.5-3b,phi35}.json`, report 3.24 | `python benchmarks/analyze_campaign.py multimodel artifacts/bounded-decode-frontier-multimodel-*.json` |
...
| B8 | **Closed for Phi-3.5 and Qwen2.5-3B** (report 3.24); Llama-3.1-8B (4-bit) is still running | cross-model generality | `artifacts/bounded-decode-frontier-multimodel-llama3.1-8b.json` when the stage lands |
