...
try:  # pragma: no cover - exercised only on a Triton-enabled GPU runner
    ...
    @triton.jit
    def _qcc_exact_global_read_kernel(
        output_ptr,
        confidence_ptr,
        query_ptr,
        key_ptr,
        ...
        """Read every exact slot without constructing a routing-index tensor."""
...
def triton_exact_global_read_chunk(
    query: torch.Tensor,
    keys: torch.Tensor,
    values: torch.Tensor,
    scores: torch.Tensor,
    ...
    if scores.shape != (batch, heads, num_sets, ways):
        raise ValueError("exact score shape does not match keys")
    if ways <= 0 or num_sets <= 0:
        raise ValueError("exact bank must contain at least one slot")
    query = query.contiguous()
    keys = keys.contiguous()
    values = values.contiguous()
    scores = scores.contiguous()
